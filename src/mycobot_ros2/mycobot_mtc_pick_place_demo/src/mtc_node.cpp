...
// MoveIt2 planning interface for trajectory execution
...
private:
  ...
  // New function to execute MTC solution using MoveGroupInterface
...
void MTCTaskNode::doTask()
{
  ...
  if (execute)
  {
    // Execute the planned task using MoveGroupInterface
    RCLCPP_INFO(this->get_logger(), "Executing the planned task with MoveGroupInterface");
    if (!executeSolutionWithMoveGroup())
    {
...
/**
 * @brief Execute MTC solution using MoveGroupInterface
 * @param solutions The MTC solution sequence to execute
 * @return true if execution succeeded, false otherwise
 */
// Include the Solution message header
 ...
 * @return true if execution succeeded, false otherwise
 */
bool MTCTaskNode::executeSolutionWithMoveGroup()
{
  if (task_.solutions().empty())
  {
    RCLCPP_ERROR(this->get_logger(), "No solutions available to execute");
    return false;
  }
  ...
  // Convert to message to access sub-trajectories
  moveit_task_constructor_msgs::msg::Solution solution_msg;
  solution.toMsg(solution_msg);
  
  // Get parameters
  std::string arm_group_name = this->get_parameter("arm_group_name").as_string();
  std::string gripper_group_name = this->get_parameter("gripper_group_name").as_string();
  
  try
  {
    // Create MoveGroupInterface for arm and gripper
    auto arm_move_group = std::make_shared<moveit::planning_interface::MoveGroupInterface>(
        shared_from_this(), arm_group_name);
    auto gripper_move_group = std::make_shared<moveit::planning_interface::MoveGroupInterface>(
        shared_from_this(), gripper_group_name);
    
    // Iterate through all sub-trajectories in the solution message
    size_t stage_num = 0;
    for (const auto& sub_traj_msg : solution_msg.sub_trajectory)
    {
      const auto& trajectory_msg = sub_traj_msg.trajectory;
      
      // Check if trajectory is empty (no points)
      ...
      // Determine which group to use based on the joint names in the trajectory
      std::shared_ptr<moveit::planning_interface::MoveGroupInterface> current_group;
      std::string group_name;
      
      // Simple heuristic: check if the first joint name belongs to arm or gripper
      if (!trajectory_msg.joint_trajectory.joint_names.empty())
      {
        ...
        // You might need a more robust way to check joint ownership, 
        // but for now let's assume if it contains "gripper" it's gripper, else arm
        // Or better: check against the group's joint model group
        
        const auto& arm_joints = arm_move_group->getJointNames();
        // const auto& gripper_joints = gripper_move_group->getJointNames(); // Unused
        
        bool is_arm = false;
        for(const auto& j : arm_joints) { if(j == first_joint) { is_arm = true; break; } }
      ...
      // HACK: Update the first point of the trajectory to match the current robot state
      // This prevents "Invalid Trajectory: start point deviates from current robot state" errors
      // which happen when the previous execution didn't settle exactly at the planned goal
      auto modified_trajectory = trajectory_msg;
      if (!modified_trajectory.joint_trajectory.points.empty())
      {
        // Get current joint positions for this group
        std::vector<double> current_joint_values;
        // We need to be careful to get joints in the same order as trajectory
        ...
        // current_group->getStartState(); // This refreshes the state monitor
        ...
        if (update_success && current_joint_values.size() == modified_trajectory.joint_trajectory.points[0].positions.size())
        {
            ...
            // Clear velocities/accelerations at start to avoid jumps
      ...
      {
        RCLCPP_ERROR(this->get_logger(), "Execution failed for %s (error code: %d)", 
                     stage_info.c_str(), result.val);
        return false;
      }
...
int main(int argc, char** argv)
{
  ...
  try {
    ...
    // Spin the executor in a separate thread to allow callbacks to be processed
    // while doTask() is running (which blocks the main thread)
    ...
    try {
      ...
      // Wait for the spin thread to finish (which happens on shutdown)
      spin_thread.join();
